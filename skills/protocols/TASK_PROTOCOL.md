...
Cross-repository findings are governed by the [Foundation Architecture](../contracts/FOUNDATION_ARCHITECTURE.md): they remain non-authoritative continuity evidence until the future owner repository is explicitly bound and the finding is freshly revalidated. They never auto-create tasks, mutate sibling repositories, or rebind the active target.

## Organizational roles and acceptance ownership

There are two organizational roles: Architect and Executor. Architect is ChatGPT. Sync, read-only Researcher, reviewer, verifier, red-team, debugger, coder, and similar modes are Executor specializations, not additional organizational roles. Specialization never changes artifact ownership, mutation authority, task authority, review authority, or acceptance ownership.

Exactly one current governing Architect owns final ACCEPT/REJECT/REVISE judgment for the active repository binding. Executor-specialized advisory sessions produce advisory evidence unless the target explicitly designates verifier-owned authoritative PASS/FAIL. They never become a second Architect. Existing v3 `review.yaml` values remain `ACCEPTED`, `REVISION_REQUIRED`, and `BLOCKED` for backward compatibility.
...
Any external repository used as normative authority must resolve to an immutable revision before mutation. Research/reference evidence, indexed or loaded capability content, synchronized snapshots, and popularity do not become normative authority automatically. Detailed source/index/admission/loading distinctions belong to the [Foundation Architecture](../contracts/FOUNDATION_ARCHITECTURE.md).
...
## Execution capability and authority boundary

Execution routing preserves the L0 invariant: required semantic capability → current authority → an authorized currently available surface. Provider/tool identity, known capability, installation, loaded content, quota, or price never grants authority; a currently available capability still requires the exact current binding. `least-powerful` and `bounded escalation` remain safety constraints.

The [Foundation Architecture](../contracts/FOUNDATION_ARCHITECTURE.md) owns capability discovery/index/source/admission/loading distinctions and control-plane routing. Operator attention/manual labor is a constrained resource, but manual convenience and cheaper execution never justify weaker correctness, safety, identity, or evidence. A platform/tool safety block is capability evidence, not permission to bypass controls. Material paid-cost approval remains separate authority.

Exactly one authorized content writer owns an active mutation boundary. Immutable publication handoff, exact-commit/content-identity publication, consequence-boundary freshness, and post-publication proof remain L0 authority/evidence obligations; changing surfaces never lowers them.

GitHub Actions must not become an iterative debugger. Tool availability is not permission to consume quota.
