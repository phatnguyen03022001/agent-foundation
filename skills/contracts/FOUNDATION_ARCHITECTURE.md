...
Attempt records have authority `NONE` and live only in repository-local Git metadata resolved through `git rev-parse --git-path`. A `RUNNING` attempt may carry at most one bounded unresolved Execution Slice in that same record; its caller-known intent is persisted before consequence-bearing dispatch, and ambiguous mutating response state is `OUTCOME_UNKNOWN` until fresh consequence-specific reconciliation establishes more. Slice telemetry is not a second store, transcript, project document, remote truth, task state, or fourth layer. Recovery still requires fresh canonical remote, HEAD, index/worktree, checkpoint, task-authority, and affected postcondition inspection. No daemon, background heartbeat, scheduler, automatic recovery, or Agent Runtime service is introduced.

Carrier routing preserves Execution Bundle while prioritizing correctness, then survivability/recoverability, then boundedness, then round-trip minimization. The current Foundation contract does not claim caller-keyed lost-ACK recovery for unkeyed Runtime `terminal_start`; survival-sensitive async mutation that is not independently reconcilable fails closed until an authorized recoverable carrier exists or an already-authorized expected-state/bounded alternative preserves proof.
