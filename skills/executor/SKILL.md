...
Use [Execution Attempt Continuity](../contracts/EXECUTION_CONTINUITY.md) when cross-session recovery needs a caller-known identity or local work hint that exact task, Git, and selected Runtime evidence cannot otherwise reconstruct. Duration alone does not require an attempt. When needed, create one local `RUNNING` attempt after exact binding, then heartbeat or checkpoint only at material work boundaries. There is no background heartbeat. When task authority permits commits and a coherent implementation boundary is already valid, prefer an early coherent Git checkpoint over carrying a large valid dirty worktree solely while waiting for broader verification.
...
On successor recovery, inspect attempt telemetry before discarding local work, but treat it only as `authority NONE` guidance. Recovery still fresh-resolves canonical remote truth and freshly inspects local HEAD, index/worktree, Git checkpoints, and current task/revision/base authority where those facts govern the next consequence. If one unresolved Execution Slice exists, resolve its selected carrier's keyed identity and typed effect first, then inspect any external consequence the carrier cannot prove. `INTENT_RECORDED` or acknowledgement without a trustworthy result means `MAY_HAVE_BEEN_DISPATCHED / OUTCOME_UNKNOWN`, never failure or safe retry. Attempt/slice telemetry never authorizes those consequences.

Before a consequence-bearing dispatch whose caller-known identity or intent is needed for cross-session recovery, persist the one current slice intent before the request can leave the caller. Read-only work and consequences already reconstructible from immutable authority plus a typed carrier do not need duplicate slice telemetry. For a durable Runtime start, retain `start_identity` in the bounded intent; let Runtime resolve process outcome. Do not invent a durable `DISPATCHED` write around a network/tool call. Clear the slice after its result/postcondition is established and a coherent recovery checkpoint exists; a typed proven result does not need a second independent process reconciliation. Raw argv, environment, secrets, prompts/transcripts, raw logs, full diffs, file bodies, and broad inventories do not belong in slice telemetry.
...
A logical deterministic suffix may be split across more than one Agent Runtime invocation only at an existing semantic, authority, capability, failure, or terminal boundary, or when one invocation cannot safely and truthfully carry the jobs because of a concrete bounded execution limit: timeout, output bound, command-size or tool limitation, or a genuinely interactive or long-running requirement. Name the reason for every such split; convenience or a desire to inspect each successful result is insufficient. For long, duration-uncertain, or survival-sensitive non-interactive work, inspect the installed `runtime_capabilities` and selected ToolContract/effect semantics, then use `terminal_start` with retained `start_identity`, `mode=pipe`, and `durability=runtime_restart` when that exact carrier is authorized and available. Default PTY and process-local starts are not cross-restart recoverable. Poll the keyed operation after a lost acknowledgement; stop on unknown or corrupt retained state, and do not replay a mutation from an unknown result. Interactive work needs a carrier that actually meets its interaction and survival requirements. These rules do not add a bundle API or runtime orchestration capability.
...
Canonical new reports omit reconstructible execution transcript and empty ceremony: repeated preflight attestations, repeated skill lists, commit narration, working-tree summaries, and absent gaps/deviations/blockers stay out when exact task/Git/verifier evidence already carries the fact. `acceptance_evidence` is the AC-to-evidence index. Include `executor_checks` only for task-specific required predicates that are distinct from the AC mapping; a verification command may be cited by an AC without a second CHECK entry. Include a compatibility field when it is materially needed; do not delete required identity, acceptance evidence, candidate/publication identity, or truthful blockers/deviations.
...
When an explicit performance audit also uses an Execution Attempt, opt into [Execution Attempt Continuity](../contracts/EXECUTION_CONTINUITY.md) performance segments only at semantic phase/activity boundaries. Use only `PREFLIGHT`, `IMPLEMENTATION`, `VERIFICATION`, `REPORTING`, and `PUBLICATION`, with activity `CONTROLLER_OWNED` or `EXTERNAL_WAIT`. `CONTROLLER_OWNED` includes controller reasoning and synchronous work but MUST NOT be described as hidden model thinking/model-active/inference time. Represent one long recoverable process or remote wait as one `EXTERNAL_WAIT` interval rather than per-poll or per-tool markers. Missing or incomplete coverage remains `UNATTRIBUTED`; never backfill it from Git/chat/tool-history timestamps. Performance attribution remains authority `NONE` and cannot alter execution, retry, acceptance, publication, or report semantics.
