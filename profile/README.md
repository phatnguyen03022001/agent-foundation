...
2. Read profile/.agent/bootstrap/bootstrap.json at F. Its repository-root-relative authority lock contains only independently versioned external authorities; Foundation-internal domains inherit F.
...
4. Resolve the bootstrap-known Case Router from `skills/.agent/case-router.yaml` at F before ordinary capability selection. BOOTSTRAP is pre-router, not a CASE.
5. Select only the admitted `EXECUTE` CASE, which routes to `executor`, then resolve each Foundation-internal capability by its domain/path at F; only external routes use an independent lock revision. Missing/unresolvable/malformed inputs and unknown cases fail closed with no mutable-ref fallback.
...
The bootstrap files are static locators and validation inputs, not a registry, daemon, cache, execution engine, or duplicated copy of support-repository semantics. Do not preload every Foundation domain or external repository, all calibration, raw chat history, historical tasks, or broad repository context by default.
...
- Keep generic governance with the Foundation `skills/` domain and target-specific truth with the target repository.
